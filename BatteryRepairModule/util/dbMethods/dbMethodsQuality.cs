...
public static partial class dbMethods
{
    ...
    public static void loadStaffQuality()
    {
        using (var conn = new NpgsqlConnection(dbConnection.connectionPath))
        using (var cmd = new NpgsqlCommand("SELECT DISTINCT \"id\", \"name\" FROM public.staff WHERE quality = true ORDER by name ASC", conn))
        {
            conn.Open();
